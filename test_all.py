...
sys.path.append('../')
from dataloader import SCIGraphDataSet,PeerReadDataSet
...
def load_AASC_graph_data(dirpath):
    #label_vocab = build_label_vocab(data_dir)
    #unknown vocavを含めることを忘れずに
    entvocab = {"UNKNOWN":0,"MASK":1}
    pathe = os.path.join(dirpath,"entity.txt")
    fe = open(pathe)
    for i,line in enumerate(fe):
        entvocab[line.rstrip("\n")] = i+2
    path_train = os.path.join(dirpath,"train.txt")
    path_test = os.path.join(dirpath,"test.txt")
    dataset_train = SCIGraphDataSet(path_train,ent_vocab=entvocab)
    dataset_test = SCIGraphDataSet(path_test,ent_vocab=entvocab)
